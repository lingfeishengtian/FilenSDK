...
final class FilenCrypto : Sendable {
     ...
     */
    func generatePasswordAndMasterKeysBasedOnAuthVersion(
        authVersion: Int,
        rawPassword: String,
        salt: String? = nil
    ) throws -> (derivedMasterKeys: String, derivedPassword: String) {
        if authVersion == 1 {
            let derivedPassword = try hashPassword(password: rawPassword)
            let derivedMasterKeys = try hashFn(message: rawPassword)
            return (derivedMasterKeys, derivedPassword)
        } else if authVersion == 2 {
            guard let salt else {
            ...
            let derivedPassword = String(derivedKey.suffix(from: middleIndex))
            let derivedMasterKeys = String(derivedKey.prefix(upTo: middleIndex))
